...
static NSString *const iRateiOSAppStoreGiftURLFormat = @"itms-appss://buy.itunes.apple.com/WebObjects/MZFinance.woa/wa/giftSongsWizard?gift=1&salableAdamId=%u&productType=C&pricingParameter=STDQ&mt=8&ign-mscache=1";
...
- (NSURL *)giftURL
{
    ...
    return [NSURL URLWithString:[NSString stringWithFormat:iRateiOSAppStoreGiftURLFormat, (unsigned int)self.appStoreID]];
...
- (void) getAppStoreID
{
	[self performSelectorInBackground:@selector(getAppStoreIDInBackground) withObject:nil];
}

- (void)getAppStoreIDInBackground
{
    @synchronized (self)
    {
        @autoreleasepool
...
- (void)openGiftAppPageInAppStore
{
    if (self.verboseLogging)
    {
        NSLog(@"iRate will open the App Store gift page using the following URL: %@", self.giftURL);
    }
    
    [[UIApplication sharedApplication] openURL:self.giftURL];
}
